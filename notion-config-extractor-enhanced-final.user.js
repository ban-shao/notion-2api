// ==UserScript==
// @name         Notion-2API 配置提取器 (增强版 v2.1)
...
(function() {
    ...
    function extractConfig() {
        ...
        if (!config.cookie) {
            ...
            console.warn('1. token_v2 Cookie 被设置为 HttpOnly，无法通过 JavaScript 访问');
            console.warn('2. 用户未登录或 Cookie 已过期');
            console.warn('3. Notion 更新了安全策略');
            console.warn('请手动从开发者工具中复制 token_v2 Cookie 值');
        } else {
...
`;
        ...
        return `# [自动提取] notion-2api 配置文件 (增强版 v2.1)
# 提取时间: ${timestamp}
${debugInfo}
# --- 安全配置 ---
API_MASTER_KEY=1
...
# 3. 请查看浏览器控制台的日志信息`;
    ...
    function extractAndShow() {
        ...
        content.innerHTML = `
            <h3 style="margin-top: 0; color: #333;">🚀 Notion-2API 配置提取器 (增强版 v2.1)</h3>
            ${statusHtml}
    ...
    function init() {
        ...
        setTimeout(() => {
            createButton();
            console.log('🚀 Notion-2API 配置提取器(增强版 v2.1)已加载');
